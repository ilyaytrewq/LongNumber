...
LongNumber::LongNumber::LongNumber(long double number, unsigned int _precision)
{
    ...
    while (mnt != 0 && bits.size() < precision)
    {
        mnt *= 2;
...
LongNumber LongNumber::operator/(const LongNumber &other) const
{
    ...
    */
    ...
    while (dividend.point < dividend.precision)
    {
        ++dividend.point;
        dividend.bits.push_back(0);
    }

    while (divisor.point < divisor.precision)
    {
        ++divisor.point;
        divisor.bits.push_back(0);
    }

    LongNumber temp(0, precision);
